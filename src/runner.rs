...
where
    ...
    E: Send + Sync + From<std::io::Error> + Debug + From<PanickedTaskError> + 'static,
{
    let AsyncCliRunner {
        context,
        task_manager,
...
where
    F: Future<Output = Result<(), E>>,
    E: Send + Sync + From<PanickedTaskError> + Debug + 'static,
{
    {
        pin_mut!(fut);
        tokio::select! {
...
impl AsyncCliRunner {
    ...
    fn new() -> Result<Self, std::io::Error> {
        ...
        let task_manager = TaskManager::new(tokio_runtime.handle().clone(), false);
...
static EXECUTOR: OnceCell<TaskExecutor> = OnceCell::const_new();
...
impl TaskExecutor {
    ...
    pub fn current() -> &'static Self {
        EXECUTOR
            .get()
            .expect("not running in a  task manager scope")
    }
